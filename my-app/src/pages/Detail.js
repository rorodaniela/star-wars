...
export default function Detail() {
    const { id } = useParams();
    const {actor, loading} = useSelector((state) => state.actorsReducer)
    const dispatch = useDispatch();

    useEffect(() => {
        dispatch(saveFetch(`id/${id}`));
    }, []);

    if (loading ) {
        return (
            <div className="Collections pt-40">
                <Loading/>
            </div>
        );
    } else {
                ...
                <div class="h-full mx-40 mt-10">
                    <div class="block md:flex max-h-96">
                        ...
                        <div
                            ...
                            className="w-full md:w-3/5 p-5 bg-white rounded lg:ml-4 shadow-md overflow-y-scroll"
                        >
